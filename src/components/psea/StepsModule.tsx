...
export function StepsModule({ city }: StepsModuleProps) {
    const steps = [
        ...
        },
        {
            icon: Wrench,
            title: "Installation",
            desc: "Pose de votre climatisation en une demi-journée, sans dégâts."
        }
    ];
                    ...
                    <div className="grid md:grid-cols-3 gap-8">
                        {steps.map((step, idx) => (
                            <div key={idx} className="relative flex flex-col items-center text-center group">
                                <div className="w-16 h-16 bg-white border-2 border-cyan-100 rounded-full flex items-center justify-center mb-4 z-10 group-hover:border-cyan-500 group-hover:bg-cyan-50 transition-colors">
