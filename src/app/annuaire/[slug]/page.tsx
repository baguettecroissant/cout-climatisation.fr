...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
    ...
    return {
        title: `Prix Climatisation ${dept.name} (${dept.code}) - Installateurs & Devis`,
        description: `Trouvez un installateur de climatisation agrée dans le ${dept.name} (${dept.code}). Devis comparatifs gratuits, aides locales (${dept.aide_locale}) et prix moyens.`,
        alternates: {
            canonical: `https://www.cout-climatisation.fr/annuaire/${slug}`,
        },
    };
}
            ...
            <section className="bg-slate-900 text-white relative pb-32 pt-12">
                <div className="container mx-auto px-4 relative z-10">
                    ...
                    <div className="text-center mt-4">
                        <div className="inline-block bg-white/10 text-cyan-300 px-3 py-1 rounded-full text-sm font-medium mb-6 border border-white/10">
                            Département {dept.code}
                        </div>
                        <h1 className="text-4xl md:text-5xl font-bold mb-6">
                            Installateurs Climatisation <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-sky-400">{dept.name}</span>
                        </h1>
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                                {neighborDepts.map((d) => (
                                    <Link
                                        ...
                                        className="flex items-center justify-between p-4 bg-white border border-slate-200 rounded-xl hover:border-cyan-500 hover:shadow-md transition-all group"
                                    >
                                        <span className="font-medium text-slate-700 group-hover:text-cyan-700">{d.name}</span>
                                        <span className="text-sm text-slate-400 bg-slate-50 px-2 py-1 rounded group-hover:bg-cyan-50 group-hover:text-cyan-600">{d.code}</span>
                                    </Link>
                                ))}
                            </div>
                        </div>
